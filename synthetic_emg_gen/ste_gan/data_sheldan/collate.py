...
def ste_gan_collate(
    ...
    for sample in batch:
        ...
        file_ids.append(sample[DataType.FILE_IDS])

    
    train_feature_length = emg_train_length // hopsize
    # Collate the features
    speech_units, phonemes, session_indices, speaking_mode_indices, start_indices, mfccs, file_ids\
        = prepare_ste_gan_features(speech_units, phonemes, session_indices, speaking_mode_indices, partition, mfccs, file_ids,
...
def prepare_ste_gan_features(speech_units_list, phonemes_list, session_indice_list, speaking_mode_id_list, partition,
                             mfccs_list, file_ids,
    ...
    session_indices = torch.stack(session_indice_list, axis=0)
    speaking_modes = torch.stack(speaking_mode_id_list, axis=0)
    file_ids = torch.tensor(file_ids, dtype=torch.int)


    # '''sheldan: for EMG names'''
    # session_id_list_ = []
    # for session_id in session_id_list:
    #     parts = session_id.split('_')
    #     numeric_part = parts[-1].split('-')
    #     num1 = int(numeric_part[0])  # First number
    #     num2 = int(numeric_part[1])  # Second number
    #     tensor = torch.tensor([num1, num2])
    #     session_id_list_.append(tensor)
    # session_ids = torch.stack(session_id_list_, axis=0)
    ...
    return speech_units, phonemes, session_indices, speaking_modes, start_idxs, mfccs, file_ids
