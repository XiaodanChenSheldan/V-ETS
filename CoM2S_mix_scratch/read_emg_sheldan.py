...
flags.DEFINE_list('silent_data_directories', None, 'silent data locations')
# flags.DEFINE_list('silent_data_directories', ['./emg_data/silent_parallel_data'], 'silent data locations')
flags.DEFINE_list('voiced_data_directories', ['./emg_data_all/voiced_parallel_data'], 'voiced data locations')
flags.DEFINE_string('text_align_directory', 'text_alignments_all', 'directory with alignment files')
# flags.DEFINE_list('voiced_data_directories', ['./emg_data_sg/voiced_parallel_data'], 'voiced data locations')
# flags.DEFINE_string('text_align_directory', 'text_alignments_sg', 'directory with alignment files')
...
def load_utterance(base_dir, index, limit_length=False, debug=False, text_align_directory=None):
    ...
    with open(os.path.join(base_dir, f'{index}_text.txt'), 'r') as file:
        text = file.read().replace('\n', '')

    sess = os.path.basename(base_dir)
    tg_fname = f'{text_align_directory}/{sess}/{sess}_{index}_audio.TextGrid'
    # tg_fname = f'{text_align_directory}/{sess}/{sess}_{index}_audio_clean.TextGrid'
    ...
    return mfccs, emg_features, text, phonemes, emg_orig.astype(np.float32)
...
class SizeAwareSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        for idx in indices:
            ...
            #     continue
            with open(os.path.join(directory_info.directory, f'{file_idx}_text.txt'), 'r') as file:
                text = file.read()
            if not np.any([l in string.ascii_letters for l in text]):
                continue

            emg = np.load(os.path.join(directory_info.directory, f'{file_idx}_emg.npy'))
            length = len(emg)
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir=None, limit_length=False, dev=False, test=False, no_testset=False, no_normalizers=False):
        ...
        if base_dir is not None:
            directories.append(EMGDirectory(0, os.path.abspath(base_dir), False))
        else:
            if FLAGS.silent_data_directories:
        ...
        for directory_info in directories:
            for fname in os.listdir(directory_info.directory):
                if fname[-3:] == 'npy':
                    self.example_indices.append((directory_info,int(fname.split('_', 1)[0])))
        ...
        total_examples = len(self.example_indices)
        train_end = int(total_examples-(200+99))
        dev_end = train_end + 200

        if (not dev) and (not test):
            self.example_indices = self.example_indices[:dev_end]
            # self.example_indices = self.example_indices*2
            # self.example_indices = self.example_indices[:4822]
        elif dev:
            self.example_indices = self.example_indices[train_end:dev_end]
        elif test:
            self.example_indices = self.example_indices[dev_end:]
        else:
        ...
        sample_mfccs, sample_emg, _, _, _ = load_utterance(self.example_indices[0][0].directory, self.example_indices[0][1])
        self.num_speech_features = sample_mfccs.shape[1]
        self.num_features = sample_emg.shape[1]
        '''Sheldan'''
        self.num_emg = sample_emg.shape[0]
        self.num_mfccs = sample_mfccs.shape[0]
        print(f'{self.num_emg=}, {self.num_mfccs=}')
        ...
        '''Sheldan'''
        self.exp_idx = {example_indices_index:example_indices_i for example_indices_index, example_indices_i in enumerate(self.example_indices)}
    ...
    def __getitem__(self, i):
        ...
        mfccs, emg, text, phonemes, raw_emg = load_utterance(directory_info.directory, idx, self.limit_length, text_align_directory=self.text_align_directory)
        ...
        result = {'audio_features':torch.from_numpy(mfccs).pin_memory(), 'emg':torch.from_numpy(emg).pin_memory(), 'text':text, 'text_int': torch.from_numpy(text_int).pin_memory(), 'file_label':idx, 'session_ids':torch.from_numpy(session_ids).pin_memory(), 'silent':directory_info.silent, 'raw_emg':torch.from_numpy(raw_emg).pin_memory()}
        ...
        if directory_info.silent:
            ...
            voiced_mfccs, voiced_emg = mfccs, emg
