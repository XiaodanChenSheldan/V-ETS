...
FLAGS = flags.FLAGS
flags.DEFINE_list('remove_channels_baseline', [], 'channels to remove')
# flags.DEFINE_list('silent_data_directories_baseline', ['./emg_data/silent_parallel_data'], 'silent data locations')
flags.DEFINE_list('voiced_data_directories_baseline', ['./emg_data/voiced_parallel_data','./emg_data/nonparallel_data'], 'voiced data locations')
# flags.DEFINE_list('voiced_data_directories_baseline', ['./emg_data/voiced_parallel_data'], 'voiced data locations')
flags.DEFINE_string('testset_file_baseline', '/data/xiaochen99/silent_speech/testset_largedev.json', 'file with testset indices')
flags.DEFINE_string('text_align_directory_baseline', '/data/xiaochen99/silent_speech/text_alignments', 'directory with alignment files')
...
def load_utterance(base_dir, index, limit_length=False, debug=False, text_align_directory=None):
    ...
    for c in FLAGS.remove_channels_baseline:
        emg[:,int(c)] = 0
        emg_orig[:,int(c)] = 0
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir=None, limit_length=False, dev=False, test=False, no_testset=False, no_normalizers=False):
        ...
        self.text_align_directory = FLAGS.text_align_directory_baseline

        if no_testset:
            devset = []
            testset = []
        else:
            with open(FLAGS.testset_file_baseline) as f:
        ...
        if base_dir is not None:
            ...
            # for sd in FLAGS.silent_data_directories_baseline:
            #     for session_dir in sorted(os.listdir(sd)):
            #         directories.append(EMGDirectory(len(directories), os.path.join(sd, session_dir), True))

            # has_silent = len(FLAGS.silent_data_directories_baseline) > 0
            has_silent = False
            for vd in FLAGS.voiced_data_directories_baseline:
        ...
        #     self.example_indices = self.example_indices[:1608]
