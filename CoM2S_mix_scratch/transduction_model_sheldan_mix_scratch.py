...
from read_emg_sheldan import EMGDataset, SizeAwareSampler
# from read_emg_ori import EMGDataset as EMGDataset_baseline
# from read_emg_ori import SizeAwareSampler as SizeAwareSampler_baseline
from read_emg_ori_nosilent import EMGDataset as EMGDataset_baseline
from read_emg_ori_nosilent import SizeAwareSampler as SizeAwareSampler_baseline
...
# flags.DEFINE_string('start_training_from', 'transduction_model.pt', 'start training from this model')
# flags.DEFINE_string('start_training_from', 'output_sheldan_mix_continue_2/model.pt', 'start training from this model')
flags.DEFINE_float('data_size_fraction', 1.0, 'fraction of training data to use')
# flags.DEFINE_float('data_size_fraction', 0.1, 'fraction of training data to use')
...
flags.DEFINE_string('output_directory', 'output_sheldan_mix_scratch_voiced', 'output directory')

def test(model, testset, self_generated_devset, device):
    model.eval()

    dataloader = torch.utils.data.DataLoader(testset, batch_size=32, collate_fn=testset.collate_raw)
    dataloader_sg = torch.utils.data.DataLoader(self_generated_devset, batch_size=32, collate_fn=self_generated_devset.collate_raw)
    losses = []
    accuracies = []
    phoneme_confusion = np.zeros((len(phoneme_inventory),len(phoneme_inventory)))
    seq_len = 200
    with torch.no_grad():
        for batch in tqdm.tqdm(dataloader, 'Validation in GT', disable=None):
...
def train_model(trainset, self_generated_trainset, devset, self_generated_devset, device, save_sound_outputs=True):
    n_epochs = FLAGS.epochs

    if FLAGS.data_size_fraction >= 1:
        training_subset = trainset.subset(FLAGS.data_size_fraction)
        self_generated_trainset_subset = self_generated_trainset.subset(FLAGS.data_size_fraction)
    else:
        training_subset = trainset.subset(FLAGS.data_size_fraction*0.1)
        self_generated_trainset_subset = self_generated_trainset.subset(FLAGS.data_size_fraction)


    dataloader = torch.utils.data.DataLoader(training_subset, pin_memory=(device=='cuda'), collate_fn=devset.collate_raw, num_workers=0, batch_sampler=SizeAwareSampler_baseline(training_subset, 256000))
    dataloader_sg = torch.utils.data.DataLoader(self_generated_trainset_subset, pin_memory=(device=='cuda'), collate_fn=devset.collate_raw, num_workers=0, batch_sampler=SizeAwareSampler(self_generated_trainset_subset, 256000))
    ...
    if FLAGS.start_training_from is not None:
        state_dict = torch.load(FLAGS.start_training_from, weights_only=True)
    ...
    # seq_len = 200 #input sequence length: 200*8=1600 (2s * 800Hz); output sequence length: 200 (2s*100Hz)
    seq_len = 2000  #input sequence length: 2000*8=16000 (20s * 800Hz); output sequence length: 2000 (20s*100Hz)

    batch_idx = 0
    for epoch_idx in range(n_epochs):
        losses = []
        for (batch_sg, batch) in tqdm.tqdm(itertools.zip_longest(dataloader_sg, dataloader), disable=None, desc='Train step'):       
            '''self generated'''
            if batch_sg is None:
                dataloader_sg_iter = iter(dataloader_sg)
                batch_sg = next(dataloader_sg_iter)
            optim.zero_grad()
            schedule_lr(batch_idx)

            X = combine_fixed_length([t.to(device, non_blocking=True) for t in batch_sg['emg']], seq_len)
            X_raw = combine_fixed_length([t.to(device, non_blocking=True) for t in batch_sg['raw_emg']], seq_len*8)
            sess = combine_fixed_length([t.to(device, non_blocking=True) for t in batch_sg['session_ids']], seq_len)

            pred, phoneme_pred = model(X, X_raw, sess)

            loss, _ = dtw_loss(pred, phoneme_pred, batch_sg)
            losses.append(loss.item())
        ...
        val, phoneme_acc, _ = test(model, devset, self_generated_devset, device)
        lr_sched.step(val)
...
def main():
    ...
    logging.info(f'FOR VERIFICATION: {len(trainset)=}, {len(devset)=}, {len(testset)=}')

    trainset_baseline = EMGDataset_baseline(dev=False,test=False)
    devset_baseline = EMGDataset_baseline(dev=True)
    testset_baseline = EMGDataset_baseline(test=True)
    logging.info('output example: %s', devset_baseline.example_indices[0])
    logging.info('train / dev split: %d %d',len(trainset_baseline),len(devset_baseline))
    logging.info(f'FOR VERIFICATION: {len(trainset_baseline)=}, {len(devset_baseline)=}, {len(testset_baseline)=}')
    ...
    model = train_model(trainset_baseline, trainset, devset_baseline, devset, device, save_sound_outputs=(FLAGS.hifigan_checkpoint is not None))
